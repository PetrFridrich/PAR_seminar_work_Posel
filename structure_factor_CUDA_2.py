...
def analysis(input_path, output_path):
    ...
    for i in range(number_of_configuration):
        ...
        el = all_configuration[i][1].astype(bool)
        el_n = np.logical_not(el)
        ...
        A_coordinates = coordinates[el_n]
        B_coordinates = coordinates[el]

        G_q_vec = cuda.to_device(q_vec)

        G_A_coordinates = cuda.to_device(A_coordinates)
        G_B_coordinates = cuda.to_device(B_coordinates)

        G_current_S_q_A = cuda.to_device(current_S_q_A)
        G_current_S_q_B = cuda.to_device(current_S_q_B)


        compute_sf[BPG, TPB](G_q_vec, G_A_coordinates, G_current_S_q_A)
        compute_sf[BPG, TPB](G_q_vec, G_B_coordinates, G_current_S_q_B)
...
@cuda.jit
def compute_sf(q_vec,coordinates,S_q):
    ...
    if q < TPB:
        ...
        for bead in coordinates:
            ...
            sp = (q_vec[q, 0] * bead[0]) + (q_vec[q, 1] * bead[1])

            cos = cos + math.cos(sp)
            sin = sin + math.sin(sp)

        S_q[q] = (cos * cos + sin * sin) / coordinates.shape[0]
...
if __name__ == '__main__':
    ...
    np.savetxt(f'Results/times_GPU_2.txt', np.array(times))