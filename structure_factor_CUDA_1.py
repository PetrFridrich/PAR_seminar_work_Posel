...
def analysis(input_path, output_path):
    ...
    number_of_chains = int(AB_ratio.shape[0])  # number of chains
    ...
    NoA_NoB = [np.sum(AB_ratio[:,0]),np.sum(AB_ratio[:,1])]

    number_of_chains_array = np.linspace(0,number_of_chains-1,number_of_chains,dtype=int)
    ...
    for i in range(number_of_configuration):
        ...
        current_S_q_A = np.zeros_like(S_q_A)
        current_S_q_B = np.zeros_like(S_q_B)

        # rebuilding chains
        coordinates = rebuild_coordinates(coordinates, Lx, Ly, Lz, number_of_chains, chain_sizes, AB_ratio, N)
        coordinates = np.asarray(coordinates)

        G_q_vec = cuda.to_device(q_vec)
        G_coordinates = cuda.to_device(coordinates)
        G_number_of_chains_array = cuda.to_device(number_of_chains_array)
        G_chain_sizes = cuda.to_device(chain_sizes)
        G_AB_ratio = cuda.to_device(AB_ratio)
        G_NoA_NoB = cuda.to_device(NoA_NoB)
        G_shifts = cuda.to_device(shifts)
        G_current_S_q_A = cuda.to_device(current_S_q_A)
        G_current_S_q_B = cuda.to_device(current_S_q_B)

        compute_sf[BPG,TPB](G_q_vec,G_coordinates,G_number_of_chains_array,G_chain_sizes,G_AB_ratio,G_NoA_NoB,G_shifts,G_current_S_q_A,G_current_S_q_B)

        current_S_q_A = G_current_S_q_A.copy_to_host()
        current_S_q_B = G_current_S_q_B.copy_to_host()
...
@cuda.jit
def compute_sf(q_vec,coordinates,number_of_chains, chain_sizes, AB_ratio,NoA_NoB,shifts,current_S_q_A,current_S_q_B):
    ...
    if q < TPB:
        ...
        for i in number_of_chains:
            ...
            for j in range(AS, AF):
                ...
                cos_A = cos_A + math.cos(sp)
                sin_A = sin_A + math.sin(sp)

            for j in range(BS, BF):
                index = shifts[i] + j

                sp = (q_vec[q, 0] * coordinates[index, 0]) + (q_vec[q, 1] * coordinates[index, 1])
                cos_B = cos_B + math.cos(sp)
                sin_B = sin_B + math.sin(sp)

        current_S_q_A[q] = (cos_A * cos_A + sin_A * sin_A) / NoA_NoB[0]
        current_S_q_B[q] = (cos_B * cos_B + sin_B * sin_B) / NoA_NoB[1]
...
if __name__ == '__main__':
    ...
    np.savetxt(f'Results/times_GPU_1.txt', np.array(times))